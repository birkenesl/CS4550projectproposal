...
function UsersNew() {
  ...
  const [user, setUser] = useState({name: "", pass1: "", pass2: "", lat: "", long: ""});
  ...
  // testing geolocation api code
  function updateLocation() {
    let u1 = Object.assign({}, user);
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(function(position) {
        u1.lat = position.coords.latitude;
        u1.long = position.coords.longitude;
      });

    }
    // location data for a user is optional, not necessary
  ...
  function onSubmit(ev) {
    ev.preventDefault();
    updateLocation(); // check location here once they submit
    console.log(user);

    let data = pick(user, ['name', 'password', 'lat', 'long']);
